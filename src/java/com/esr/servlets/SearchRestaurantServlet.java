...
public class SearchRestaurantServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            res = d.searchRestaurant(name);
            if (res == null) {
                out.println("<h2> unable to read</h2>");
            }            
            else{
                HttpSession s = request.getSession();
                s.setAttribute("searchRes", res);
                response.sendRedirect("searchRestaurant.jsp");
            }
            ...
            Logger.getLogger(SearchRestaurantServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
